...
class Onda:
    ...
    def __init__ (self,Amplitud=1,Angulo=0,Phase=0,Tipo='Lineal',Frecuencia=Parametros['Frecuencia'],Color='b',Resultante=False,Fuentes=[],Mostrar=True,ZInicial=Parametros['ZInicial'],ZFinal=Parametros['ZFinal'],Nombre='SinNombre',Exc=0.5):
        self.Nombre=Nombre
    ...
    def CalculoFuncionCompleta(self,t=0):
        if not self.Resultante: 
            FuncionCompleta=[]
            # Calcula las amplitudes iniciales
            ...
            if self.Tipo=='Lineal':
                ...
                #La p representa una prima que es el sistema rotado de tal manera que x e y sean los semiejes mayor y menor
                Axp=self.Amplitud
                Ayp=self.Amplitud*self.Excentricidad
                Axpx=Axp*np.cos(self.Angulo)
                Axpy=Axp*np.sin(self.Angulo)
                Aypx=-Ayp*np.sin(self.Angulo)
                Aypy=Ayp*np.cos(self.Angulo)
                # Ahora debe considerar la amplitud compleja y sumarla en los ejes X e Y sin primar
                if Axpx<0:
                    Cxpx=cmath.rect(-Axpx, Phasex)
                else:
                    Cxpx=cmath.rect(Axpx, Phasex)
                if Axpy<0:
                    Cxpy=cmath.rect(-Axpy, Phasex)
                else:
                    Cxpy=cmath.rect(Axpy, Phasex)
                if Aypx<0:
                    Cypx=cmath.rect(-Aypx, Phasey)
                else:
                    Cypx=cmath.rect(Aypx, Phasey)
                if Aypy<0:
                    Cypy=cmath.rect(-Aypy, Phasey)
                else:
                    Cypy=cmath.rect(Aypy, Phasey)
                Cx=Cxpx+Cypx
                Cy=Cxpy+Cypy
                # Recupera por separado la amplitud y la fase
                Ax,Phasex=cmath.polar(Cx)
                Ay,Phasey=cmath.polar(Cy)
                
            # Calcula las fases iniciales
            if self.Tipo=='Lineal':
                Phasex=self.Phase
                Phasey=self.Phase
            if self.Tipo=='Circular+':
...
def CrearOnda(Amplitud=1,Angulo=0,Phase=0,Tipo='Lineal',Frecuencia=Parametros['Frecuencia'],Color='b',Resultante=False,Fuentes=[],Mostrar=True,ZInicial=Parametros['ZInicial'],ZFinal=Parametros['ZFinal'],Nombre='SinNombre',Exc=1):
        OndaNueva=Onda(Amplitud,Angulo,Phase,Tipo,Frecuencia,Color,Resultante,Fuentes,Mostrar,ZInicial,ZFinal,Nombre,Exc)
...
def Ejemplo(n):
    ...
    #
    if n==1:
        CrearOnda(Tipo='Circular-',Exc=0.5,Angulo=np.pi/4)
        #CrearObjeto('Polarizador',15,Angulo=np.pi/16)


#Ejemplo(1)


CrearOnda(Tipo='Lineal',Angulo=np.pi/2,Nombre='X',Phase=np.pi/2,Mostrar=False)
CrearOnda(Tipo='Lineal',Nombre='Y',Color='red',Mostrar=False)
CrearOnda(Resultante=True,Fuentes=['X','Y'],Color='y')
#CrearObjeto(Tipo='Polarizador', Posicion=15, Angulo=np.pi/3)
CrearObjeto(Tipo='Lamina',Posicion=20,Largo=9,IndiceX=2,IndiceY=3,Color='green',Borde=False)
